...
class IcebergManager:
    ...
    def writeDfToTable(self, df, table_name):
        if self.spark.catalog.tableExists(table_name):
            df.writeTo(table_name).overwritePartitions()
        else:
            df.writeTo(table_name).partitionedBy('date').createOrReplace()

    def create_table_to_merge(self, table_name, from_table):
        self.spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {catalog_name}.{database_name}.{table_name}
        USING iceberg
        PARTITIONED BY (date)
        AS SELECT * from {catalog_name}.{database_name}.{from_table}
                 """)

    def merge_table(self, table_name, from_table):
        self.spark.sql(f"""
        MERGE INTO {catalog_name}.{database_name}.{table_name} t
        USING (SELECT * from {catalog_name}.{database_name}.{from_table}) s
        ON t.date = s.date and t.client_id = s.client_id
        WHEN MATCHED THEN UPDATE SET t.cl_count_of_hire_v1 = s.cl_count_of_hire_v1
...
if __name__ == "__main__":
    ...
    table_name = "acl_assignments_outcomes_partitioned"
    copy_to_table_name = "acl_assignments_outcomes_target"

    # Instantiate the manager and perform operations
    manager = IcebergManager(catalog_name, database_name, warehouse_path)
    df = manager.compute('2024-02-08', '2024-02-12')
    df.filter(col('cl_count_of_hire_v1')>0).show(10, False)
    manager.writeDfToTable(df, f"{catalog_name}.{database_name}.{table_name}")
    logger.info("creating table to merge")
    manager.create_table_to_merge(copy_to_table_name, table_name)
    logger.info("Merging...")
    manager.merge_table(copy_to_table_name, table_name)
